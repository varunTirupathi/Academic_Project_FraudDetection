...
def stopStartStreamingJob():
    ...
    start_streaming = 'spark-submit --class com.project.spark.jobs.RealTimeFraudDetection.DstreamFraudDetection --name "RealTime Creditcard FraudDetection" --master spark://project:6066 --deploy-mode cluster  --total-executor-cores 1' + ' ' + home + '/frauddetection/spark/fruaddetection-spark.jar' + ' ' + home + '/frauddetection/spark/application-local.conf'
...
with DAG(DAG_NAME,
    ...
    create_model = BashOperator(
        ...
        bash_command='spark-submit --class com.project.spark.jobs.FraudDetectionTraining --name "Fraud Detection Spark ML Training" --master spark://project:7077  --total-executor-cores 1' + ' ' + home + '/frauddetection/spark/fruaddetection-spark.jar' + ' ' + home + '/frauddetection/spark/application-local.conf',
        default_args=default_args)
